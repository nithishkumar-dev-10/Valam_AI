...
2. **Apply config to the server:**
   - Copy the updated section defaults from `backend/.env.example` (`ACCESS_TOKEN_EXPIRE_MINUTES=15`, `MAX_IMAGE_UPLOAD_MB=10`) and deploy new `deploy/nginx-valam.conf` (`nginx -t && systemctl reload nginx`). Run `pip install -r backend/requirements.txt` (new fastapi/starlette) and restart `valam-backend.service`. The `RefreshToken` table is created automatically at startup; **back up `valam.db` before the first run** of the updated backend.
