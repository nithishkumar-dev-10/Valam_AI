...
import { CropCard } from "../components/ResultViews";
import { ConfidenceStamp, Stamp } from "../components/Stamps";
import { IconAlert, IconMapPin, IconRefresh } from "../components/Icons";
...
const FIELDS: Array<{
  ...
  { key: "temperature", label: "Temperature", unit: "°C", min: 8, max: 44, step: 0.5, decimals: 1 },
  { key: "humidity", label: "Humidity", unit: "%", min: 10, max: 100, step: 1 },
  { key: "ph", label: "Soil pH", unit: "", min: 3.5, max: 9.9, step: 0.1, decimals: 1 },
  { key: "rainfall", label: "Rainfall", unit: "mm/yr", min: 20, max: 300, step: 1 },
];
...
export function CropPage() {
    ...
    try {
      ...
      if (!coordsInIndia(latitude, longitude)) {
        toast.info(
          ...
          "Results work best within India (6°N–37.5°N, 68°E–97.5°E).",
        );
      }
    } catch (err) {
      toast.error("Couldn't get your location", apiErrorMessage(err));
      setGpsManual(true);
    } finally {
      setLocating(false);
    }
  ...
  const submit = () => {
    if (mode === "gps") {
      if (!gps) {
        toast.info("Add your location", "Tap “Use my location” or enter coordinates manually.");
        return;
      }
      ...
      <motion.div variants={staggerParent} initial="hidden" animate="show">
        <motion.div variants={fadeUp} className="text-center sm:text-left">
          ...
          <p className="mx-auto mt-2 max-w-md text-[14.5px] leading-relaxed text-sage sm:mx-0">
            A crop suggestion built from live weather and regional soil data — two ways to get there.
          </p>
        </motion.div>
        ...
        <motion.div variants={fadeUp} className="mt-6 flex justify-center sm:justify-start">
          <Segmented<Mode>
            id="crop-mode"
            options={[
              { value: "gps", label: "Use my location" },
              { value: "manual", label: "Soil measurements" },
            ]}
            value={mode}
            onChange={setMode}
          />
          ...
          {mode === "gps" ? (
            <div className="card space-y-4 p-6">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h2 className="text-[15px] font-semibold text-pine-900">Where is the field?</h2>
                  <p className="mt-0.5 text-[13px] leading-relaxed text-sage">
                    We resolve your district, pull regional soil values and grab live weather
            ...
            <div className="card space-y-5 p-6">
              <p className="text-[13px] leading-relaxed text-sage">
                Enter measured values — from a Soil Health Card or a local lab. These bypass the
                lookup and are treated as high-confidence inputs.
              </p>
              {FIELDS.map((f) => (
                <MeasureRow
                  key={f.key}
          ...
          {phase === "loading" && (
            ...
            >
              ...
              <p className="mt-3 text-center text-[13px] font-medium text-sage">
                Looking up soil, weather and models… this takes a few seconds.
              </p>
            </motion.div>
          )}
            ...
            >
              ...
              <div className="flex items-center justify-between rounded-2xl border border-mist bg-surface px-5 py-4 shadow-edge">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Stamp label={result.input_confidence ?? "—"} tone="pine" />
                </div>
